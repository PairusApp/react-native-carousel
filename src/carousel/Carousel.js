...
export default class Carousel extends Component {
  ...
  constructor(props) {
    ...
    if (!ViewPropTypes) {
      console.warn(
        "react-native-carousel: It is recommended to use at least version 0.44 of React Native with the plugin"
      );
    }
    if (!props.vertical && (!props.sliderWidth || !props.itemWidth)) {
      console.error(
        "react-native-carousel: You need to specify both `sliderWidth` and `itemWidth` for horizontal carousels"
      );
    }
    if (props.vertical && (!props.sliderHeight || !props.itemHeight)) {
      console.error(
        "react-native-carousel: You need to specify both `sliderHeight` and `itemHeight` for vertical carousels"
      );
    }
    if (props.apparitionDelay && !IS_IOS && !props.useScrollView) {
      console.warn(
        "react-native-carousel: Using `apparitionDelay` on Android is not recommended since it can lead to rendering issues"
      );
    }
    if (props.customAnimationType || props.customAnimationOptions) {
      console.warn(
        "react-native-carousel: Props `customAnimationType` and `customAnimationOptions` have been renamed to `activeAnimationType` and `activeAnimationOptions`"
      );
    }
    if (props.onScrollViewScroll) {
      console.error(
        "react-native-carousel: Prop `onScrollViewScroll` has been removed. Use `onScroll` instead"
      );
    }
  }
